...
cells.append(md(
    '# Notebook 06: Closed-Loop Simulation\n\n'
    'End-to-end integration test of the full AI pipeline **before** building the web app.\n\n'
    'This notebook is the **reference implementation** for the backend `simulation_engine.py`.\n\n'
    '**Loop:**\n'
    '1. Load a test trajectory (gengine1)\n'
    '2. Step through it window-by-window\n'
    '3. BiLSTM classifies each window\n'
    '4. If fault detected → controller proposes action → twin validates → correction applied\n'
    '5. Plot Lambda convergence and emissions over the full run',
    '0001'))
...
cells.append(md('## 6.1 Load Models', '0003'))

cells.append(code(
    '''import tensorflow as tf
tf.random.set_seed(42)

classifier = tf.keras.models.load_model(MODEL_DIR + "bilstm_classifier.h5")
twin       = tf.keras.models.load_model(MODEL_DIR + "lstm_digital_twin.h5")

print(f"BiLSTM classifier : {classifier.name}")
print(f"LSTM digital twin : {twin.name}")''',
    '0004'))

cells.append(md('## 6.2 Simulation Components', '0005'))
...
cells.append(md('## 6.3 Run Closed-Loop Simulation\n\nScenario: Inject Fault 1 (Rich Mixture) at cycle 20, then let the controller correct it.', '0007'))
...
cells.append(md('## 6.4 Plot Results', '0009'))
...
cells.append(md('## 6.5 Convergence Metrics', '0011'))
...
out = 'D:/mini project/notebooks/06_closed_loop_simulation.ipynb'
